...
一个 Chrome 侧边栏扩展，将完整浏览器和 AI 聊天平台集成到 Chrome 侧边栏中。

## 💡 使用方式

**零门槛，开箱即用。**

如果你的浏览器已经登录过某个 AI 平台（如 DeepSeek、ChatGPT、Claude 等），且登录状态未过期，打开侧边栏后**直接就能聊天**，不需要重复登录。

如果登录已过期，或者你想使用新的 AI 平台，**直接在侧边栏内登录即可**，和普通浏览器一样操作。

> 侧边栏本质上是一个内嵌浏览器，它会共享你 Chrome 主浏览器的 Cookie，所以只要主浏览器登录过，侧边栏就能直接用。

## 📸 截图

| AI 平台列表 | 聊天界面 |
|:-----------:|:--------:|
| ![AI 平台列表](screenshots/ai_list.jpg) | ![聊天界面](screenshots/deepseek.jpg) |
